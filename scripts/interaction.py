...
from label_formats import (  # noqa: F401 — re-export:app/測試沿用 interaction 入口
    _NDJSON_IMG_KEYS,
    annotation_boxes_for_image,
    annotation_image_label,
    coco_boxes_for_image,
    coco_file_for_root,
...
def draw_yolo_boxes(
    ...
    rows: list[tuple[int, float, float, float, float, str | None]] = []
    if label_path.exists():
        for line in safe_read_text(label_path).splitlines():
            parts = line.split()
            if len(parts) < 5 or len(parts) >= 7:  # >=7 欄=seg/OBB,不硬畫錯框
                continue
            try:
                cid = int(parts[0])
                ...
                continue
            rows.append((cid, cx, cy, bw, bh, None))
    else:  # 無 YOLO txt → 試 COCO/VOC/LabelMe/NDJSON(原生支援)
        rows = [(cid, cx, cy, bw, bh, cname)
                for (cid, cx, cy, bw, bh, _s, cname)
                in (annotation_boxes_for_image(image_path) or [])]
    if not rows:
        return img
    draw = ImageDraw.Draw(img)
    w, h = img.size
    for cid, cx, cy, bw, bh, cname in rows:
        x0, y0 = (cx - bw / 2) * w, (cy - bh / 2) * h
        x1, y1 = (cx + bw / 2) * w, (cy + bh / 2) * h
        draw.rectangle([x0, y0, x1, y1], outline="#e74c3c", width=2)
        if cname:
            name = cname
        elif class_names and cid is not None and 0 <= cid < len(class_names):
            name = class_names[cid]
        else:
            name = f"class_{cid}" if cid is not None else "?"
...
def parse_yolo_boxes(
    ...
    for line in safe_read_text(label_path).splitlines():
        parts = line.split()
        # >=7 欄＝segmentation 多邊形/OBB(class x1 y1 x2 y2 …)——前 4 個座標會被
        # 誤讀成 cx cy w h(silent-wrong),一律跳過;偵測框只有 5 欄(GT)或 6 欄(pred+conf)。
...
def parse_yolo_boxes_conf(
    ...
    for line in safe_read_text(label_path).splitlines():
        parts = line.split()
        if len(parts) < 5 or len(parts) >= 7:  # >=7 欄=seg/OBB,跳過(見 parse_yolo_boxes)
            continue
        try:
            cid = int(float(parts[0]))
            cx, cy, w, h = (float(v) for v in parts[1:5])
...
def has_seg_like_labels(
    image_paths: Sequence[Path],
    label_for: Callable[[Path], Path] | None = None,
    probe: int = 50,
) -> bool:
    """抽樣檢查 label 檔是否為 segmentation/OBB 格式(每行 >6 個數值欄)。
    給「找不到任何 bbox」的錯誤訊息當診斷:格式不對 vs 真的沒標註。"""
    resolve = label_for or yolo_label_path_for
    for ip in list(image_paths)[:probe]:
        lp = resolve(Path(ip))
        if not lp.exists():
            continue
        for line in safe_read_text(lp).splitlines():
            parts = line.split()
            if len(parts) >= 7:
                try:
                    float(parts[1]); float(parts[6])
                except ValueError:
                    continue
                return True
    return False
...
def discover_yolo_objects(
    ...
    for ip in image_paths:
        ip = Path(ip)
        rows = [(cid, cx, cy, w, h, score, None)
                for (cid, cx, cy, w, h, score) in parse_yolo_boxes_conf(resolve(ip))]
        if not rows:  # 無 YOLO txt(或空)→ 試 COCO/VOC/LabelMe/NDJSON;txt 永遠優先
            rows = annotation_boxes_for_image(ip) or []
        for k, (cid, cx, cy, w, h, score, cname) in enumerate(rows):
            # COCO 的 category_id 非 0 起連號 → 類名直接用標註內的 name(cname),
            # 不能拿去索引 classes.txt;YOLO txt 走原本的 names[cid]。
            if cname:
                label = cname
            elif cid is not None and names and 0 <= cid < len(names):
                label = names[cid]
            else:
                label = f"class_{cid}" if cid is not None else "unknown"
            out.append({
                "image_path": ip, "label": label, "class_id": cid,
                "bbox": (cx, cy, w, h), "obj_index": k, "score": score,
            })
    # 名稱制標註(VOC/LabelMe/NDJSON)無數字 id → 以類名穩定補號,
    # 不與既有 id 撞號(class_id 需「同類同號、跨類不撞」:autotune 純度分組用)。
    pending = [i for i, r in enumerate(out) if r["class_id"] is None]
    if pending:
        used = {r["class_id"] for r in out if r["class_id"] is not None}
        base = (max(used) + 1) if used else 0
        name_ids = {nm: base + j
                    for j, nm in enumerate(sorted({out[i]["label"] for i in pending}))}
        for i in pending:
            out[i]["class_id"] = name_ids[out[i]["label"]]
    return out
