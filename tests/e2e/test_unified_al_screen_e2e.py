...
『能分辨瑕疵種類 / 只做異常偵測』。本檔以「建模後 head 有無」作為 gate 的真實行為信號。

兩條真實行為斷言(都是查『行為差異』,非 element 存在):
  1. test_semantics_gate_blocks_head:同一份 door/window(各 10≥N_min=8)資料,
     語義=物件類別 → 建模『只做異常偵測』;語義=瑕疵類別 → 建模『能分辨瑕疵種類』。
     資料量沒變、只有語義變 → 證明是『語義守門』非『資料量不足』。
  2. test_below_nmin_no_head:scratch=7<8 的瑕疵資料、語義=瑕疵類別,建模後仍『只做異常偵測』
...
def test_semantics_gate_blocks_head(app_server, browser, yolo_object_2class):
    """door/window 各 10(≥N_min=8)。語義=物件類別 → 建模只做異常偵測;語義=瑕疵類別 → 能分辨瑕疵種類。
    資料量不變、只有語義變 → 證明是『語義守門』非『資料量不足』。
    用兩個獨立 context(各自乾淨 session)各建一次,避免狀態互相污染。"""
    ds = yolo_object_2class

    # (1) 語義=物件類別 → 建模『只做異常偵測』(物件類別不訓瑕疵 head)
    ...
    try:
        ...
        assert "只做異常偵測" in main_obj, \
            "語義=物件類別時不該訓出分類頭(語義安全鎖)"
        assert "能分辨瑕疵種類" not in main_obj, "物件類別語義下絕不應出現『能分辨瑕疵種類』"
    finally:
        ctx1.close()

    # (2) 同資料、語義=瑕疵類別 → 建模『能分辨瑕疵種類』(door/window 各 10≥8)
...
def test_below_nmin_no_head(app_server, browser, yolo_defect_below_nmin):
    """scratch=7<N_min=8、stain=9。語義=瑕疵類別。建模後仍『只做異常偵測』
